...
在不定义工具的情况下添加模型可见的请求上下文的产品插件。`agent-instructions` 包含在默认 `dsh-agent-spine-demo` 组合包中，可通过组合包配置禁用；`time-context` 由每个交付 profile 的 dsh-base 组合包挂载，可按 id 禁用或重新配置；`tmux-context` 和 `session-reference` 需主动启用。
