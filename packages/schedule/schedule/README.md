...
Time-context is not a Schedule dependency. Every shipped profile mounts `@deepseek-ai/dsh-time-context` through the dsh-base bundle so the model can interpret natural language in the browser's request-local zone; the official Schedule Web overlay clears its refresh interval so each request step gets a fresh reading. The model must still pass an explicit offset or `time_zone` to `schedule_create`; Schedule never imports or infers from model context.
